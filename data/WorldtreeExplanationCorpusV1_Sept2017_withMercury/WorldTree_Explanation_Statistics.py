...
RAW_TEXT_PATH = 'Data/explanations_plaintext.withmercury.txt'
...
def question_list_to_dict_raw(question_list_list):
  question_list_dict = list([])
  knowledge_base = list([])
  knowledge_base_dict = {}
  fact_id = 0
  ...
  for question_list in question_list_list:
    ...
    # capture the explanations of the questions
    question_dict["explanations"] = question_list[4:]

    question_list_dict.append(question_dict)

  return question_list_dict

def match_role(explanation):
  m = re.search(r'(?<=ROLE: )\w+', explanation)
  return m.group(0)

# Write a function to do the following stuff: 
...
def check_dataset_statistics():
  ...
  # convert each question to a dictionary
  question_list_dict = question_list_to_dict_raw(question_list_list)

  explanations_count_dict = {'all':0, 'CENTRAL':0, 'GROUNDING':0,'LEXGLUE':0,'BACKGROUND':0}
  explanations_length = list([])
  for i, question_dict in enumerate(question_list_dict):
    explanations_length.append(len(question_dict["explanations"]))
    for explanation in question_dict["explanations"]:
      role = match_role(explanation)
      explanations_count_dict['all']+=1
      if role in explanations_count_dict.keys():
        explanations_count_dict[role]+=1
      else:
        explanations_count_dict[role]=0

  # There seems to be roles that are not defined in the paper:
  # ROLE: this should be GROUNDING I thinkg
  # NEG: 
  # NE:
  explanations_length = np.array(explanations_length)

  print('total number of explantions:',np.mean(explanations_length))
  print('distribution of roles',explanations_count_dict)

  # plt.figure()
  # _ = plt.hist(explanations_length, bins='auto')
  # plt.show()
  return 0
